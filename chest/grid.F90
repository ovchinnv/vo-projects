...
!
      real*8, pointer, public, save, dimension(:) :: &
...
!
       ...
       integer :: bug
       real*8 :: x, y, z
       real*8 :: x0, x1, y0, y1, z0, z1, xstretch, ystretch, zstretch
       real*8 :: dxmax, dxmin, dymax, dymin, dzmax, dzmin
       logical :: qxgridobject=.false., qygridobject=.false., qzgridobject=.false.
       logical :: xstretchfix=.false., ystretchfix=.false., zstretchfix=.false.
       real*8, pointer :: dimens(:,:) ! object dimensions
       real*8, pointer :: agrid(:)
       integer :: nnx, nny, nnz, ndim
       logical :: q3d
! default parameters for automatic grid generation
!
       real*8, parameter :: dmin_default=0.5d0, stretch_default=1.05d0, buffer_default=30d0
!
...
!**********************************************
       if (qxgridobject) then
! hack into gridsize module: note that if state has been initialized, need to reinitialize; do not do this at home
        nx=nnx
       endif
!**********************************************
       if (qygridobject) then
! hack into gridsize module
        ny=nny
       endif
!**********************************************
       if (qzgridobject) then
...
!
       if (me.eq.0) then
        write(msg__,'(A)')'============== GRID INITIALIZED ==============';call message(whoami,msg__)
      write(msg__,'(A,3I5," ( ",I9,A," ) ")')'Global domain size: ',nx,ny,nz,(nx-2)*(ny-2)*(nz-2),' DOF';call message(whoami,msg__)
        fmt='(A,'//realfmt//')'
        write(msg__,fmt)'dx max    =',dxmax;call message(whoami,msg__)
        write(msg__,fmt)'dx min    =',dxmin;call message(whoami,msg__)
        write(msg__,fmt)'dy max    =',dymax;call message(whoami,msg__)
        write(msg__,fmt)'dy min    =',dymin;call message(whoami,msg__)
        if (q3d) then
         write(msg__,fmt)'dz max    =',dzmax;call message(whoami,msg__)
         write(msg__,fmt)'dz min    =',dzmin;call message(whoami,msg__)
        endif
        write(msg__,fmt)'dx/dy max =',dxmax/dymin;call message(whoami,msg__)
        write(msg__,fmt)'dx/dy min =',dxmin/dymax;call message(whoami,msg__)
        if (q3d) then
         write(msg__,fmt)'dx/dz max =',dxmax/dzmin;call message(whoami,msg__)
         write(msg__,fmt)'dx/dz min =',dxmin/dzmax;call message(whoami,msg__)
         write(msg__,fmt)'dy/dz max =',dymax/dzmin;call message(whoami,msg__)
         write(msg__,fmt)'dy/dz min =',dymin/dzmax;call message(whoami,msg__)
         write(msg__,'(A)')'=============================================';call message(whoami,msg__)
        endif
       endif
!
...
!
       real*8 :: a, b, z
       integer :: n, i
       real*8, pointer, dimension(:) :: grid_exponential, x
!
...
!
       implicit none
       real*8 :: xmin, xmax, dxmin, stretch, buffer
       logical :: stretchfix
       real*8, pointer, dimension(:) :: grid_around_object
       real*8, pointer, dimension(:) :: g
       integer :: n1, n2, n, nnew(1)
       integer, dimension(4) :: ns
       real*8 :: x, dx0, dx
       real*8 :: TOL
       real*8 :: b, c, s, ys, dyds, ys1, dyds1, oos, sn
...
! for simplicity, keep the inner grid the same, but modify stretching:
       n2=n2 + mod(n2,2) * sign(1,n-nnew(1)) ; ! make the number of inner points even if it is not

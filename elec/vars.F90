...
! electrostatic potential variables
 ...
 float :: el=0d0 ! long-range electrostatic potential
 float :: elsr=0d0 ! short-range electrostatic potential
 float :: el_self=0d0 ! self-interaction part of long-range energy
!
 __FARR(grad_el,:,:)
...
!
 ...
 __FPAR(pi,atan(1d0)*4d0)
 __FPAR(twopi,2d0*pi)
 __FPAR(fourpi,4d0*pi)
! should be softcoded below 
 __FPAR(eps,1d0); ! permittivity
 __FPAR(oeps,1d0/eps); ! permittivity
 __FPAR(spt,1d0); ! filter support -- should move this elsewhere and make soft

! for long-range ewald
  __IPAR(nwave,10) ! wavenumber range

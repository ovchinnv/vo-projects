...
//   opts.require("tabulatedEnergies", "tableNumTypes","Number of types for energy tabulation", &tableNumTypes);
  ...
  //End of symmetry restraint parameters.
   ...
   opts.optionalB("main", "freeEnergy", "Use free energy restraints?",
     &freeEnergyOn, FALSE);
   opts.require("freeEnergy", "freeEnergyConfig",
     "Configuration file for free energy restraints", PARSE_MULTIPLES);
...
//fepe
  ...
  if (LCPOOn) {
   ...
   if (freeEnergyOn)
   {
     iout << iINFO << "FREE ENERGY RESTRAINTS ACTIVE\n";
     ...
     if ( strstr(current->data,"\n") ) {
       iout << iINFO << "FREE ENERGY RESTRAINTS SCRIPT INLINED IN CONFIG FILE\n";
       continue;
     }

     iout << iINFO << "FREE ENERGY RESTRAINTS SCRIPT   " << current->data << "\n";

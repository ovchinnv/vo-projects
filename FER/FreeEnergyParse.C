...
#include "Vector.h"
//#include "FreeEnergyVector.h" ; // removed dependency on this class; using NAMD Vecrot.h
...
//----------------------------------------------------------------------------
  ...
  iout << "FreeEnergy: ";
  if (Terminate) { // this looks like a cast to essentially a one-bit value, so why have the Bool_t type in the first plac?
...
#ifdef FE_RESTRAINT_RMSD_FORTRAN
      case kRMSD:   RestraintType=kRMSDBound;   break;
#endif
      default: break;
    }
    Str += 5;
...
#ifdef FE_RESTRAINT_RMSD_FORTRAN
      case kRMSD:   RestraintType=kRMSDPMF;   break;
#endif
      default: break;
    }
    Str += 3;
...
int AddAtomsFromPDB(AGroup& Group, char* str, GlobalMasterFreeEnergy& CFE) {
//---------------------------------------------
// read PDB file name and column specification, store atom indices 
// that match the specification
// Code partially adapted from TMD Global master class
// VO: 2013
//
 enum col_t {beta, occu};
//
 str += ReadWord(str, "file");
// str += ReadWhite(str);
 str += ReadChar(str, '=');
// str += ReadWhite(str);
// file name is next; read PDB
...
//   8.  kPDBFile           file=filename, col=value // col can be beta, occu, x, y, z; value is any number
...
int IsPDBFile(char* Str) {
//------   file=filename, col=value // col can be beta, occu, x, y, z; value is any number
 char* IniStr=Str;
 int count;
 double val;
...
int ReadFileName(const char* Str) {
//-------------------------------------------------------------------

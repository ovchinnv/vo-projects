...
! reset internal interation counter for ftsm_master
       olditeration=0
!
       update_on=(remove_tag(comlyn,'UPDA',comlen).gt.0)
       if (update_on) then
...
!
       evolve_ftsm_on=(remove_tag(comlyn,'EVOL',comlen).gt.0)
...
!cccccccccccccccccc STRING METHOD OPTIONS cccccccccccccccccccccc
       if (qprint) then
        WRITE (msg___,'(2A)') whoami, ' STRING METHOD ENABLED.'; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        if (evolve_ftsm_on) then
 ...
 671 format(A,' STRING EVOLUTION WILL BE OF THE FORM:',/, &
     & A,' R(N+1)=A*R(N)+(1-A)*RINST, A=',F9.5,'.')
            elseif (evolve_aver_on) then
               write(msg___,6710) whoami, whoami, num_evolve_samples ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6710 format(A,' CV EVOLUTION WILL BE OF THE FORM:',/, &
     ...
     & write(msg___, 6711) whoami, max_evolve_samples ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6711 format(A, ' ONLY THE MOST RECENT ', I5,' SAMPLES WILL BE USED.')
            endif
        endif ! evolve_ftsm_on
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        if (update_on) then
          WRITE (msg___,666) whoami, update_freq ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 666 format(A,' WILL UPDATE IMAGES AFTER EVERY ',I7,' ITERATIONS.')
         if (repa_on) &
     & WRITE (msg___,667) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! proj_on is necessary to compute free energies
     ...
     & ' WILL RESTRAIN SYSTEM TO PLANE PERPENDICULAR TO PATH.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        else
            write (msg___,'(2A)') whoami, &
     & ' WILL RESTRAIN SYSTEM TO PATH IMAGE.'//                         &
     & ' (FE/MFPT CANNOT BE COMPUTED).' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        if (stat_on) then
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      elseif (( keyword(1:4).eq.'FILL'(1:4) )) then ! set path from current coordinates
!
       qcomp=(remove_tag(comlyn,'COMP',comlen).gt.0)
!
       if (qcomp) then
         if (qprint) write(msg___,6657) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6657 format(/A,' WILL DEFINE PATH FROM COMPARISON COORDINATES.')
         ...
         if (qprint) write(msg___,6660) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! check fd spec
        ...
        if (qprint) write(msg___, 7001) whoami,whoami,step,whoami,whoami ;do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! write(me_global+100,*) fd_error
         if (qprint) then
          if (proj_on) then
           write(msg___,7006) whoami, whoami, whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 7002 format(/A,' TOP:    NORMALIZED PROJECTION ONTO PATH', &
     ...
     & whoami,fd_error(i,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
           enddo
          else ! projection off
           write(msg___,7013) whoami, whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 7013 format(/A,' DISTANCE TO PATH POINT:', &
     & /A,' DGRAD_X_MAX, DGRAD_Y_MAX, DGRAD_Z_MAX', &
     & /A,' ======================================')
            write(msg___,'(A,3'//real_format//'F15.9)')                   &
     & whoami,fd_error(1,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
          endif ! projection
         endif ! qprint
...
!
       if (remove_tag(comlyn,'PARA',comlen).gt.0) then ! parallel communication test
        if (qprint) write(msg___, 7005) whoami,whoami,whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 ...
 7006 format(/A,' TOP:    NORMALIZED PROJECTION ONTO PATH', &
     ...
     & whoami,fd_error(i,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
           ...
           write(msg___,7010) whoami, whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       ...
       do while (comlen .gt. 1)
        ...
        select case(keyword)
           case('YES','ON','TRUE','T','yes','on','true','t')
            ...
            if (qprint) then
             write(msg___,7009) whoami, 'FORCES', keyword ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
            endif
...
! local is specified with 'COR'; global is the default
       ...
       qcor=remove_tag(comlyn,'CHM',comlen); qcor = min(qcor,1)
...
! print summary
          if (qprint) then
            write(msg___,100) whoami, norient ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 100 format(A,' WILL ORIENT STRUCTURES BASED ON ',i5,' ATOMS')
            write(msg___,101) whoami ; ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 101 format(A,' ORIENTATION WEIGHTS UNIFORM.')
            if (qdiffrot) then
             write(msg___,102) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
            else
             write(msg___,103) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
            endif
...
! print summary
          if (qprint) then
            write(msg___,104) whoami, nforced ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 104 format(A,' WILL APPLY FORCES TO ',i5, &
     & ' ATOMS')
            write(msg___,105) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
!***************************************************************
 ...
 natom=psf_natom()
          select case(keyword(1:klen))
           case('YES','ON','TRUE','T','yes','on','true','t')
            if (qprint) then ; write(msg___,8001) whoami, 'SET FROM ATOM MASSES' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
            call ftsm_set_weights(m, natom) ! send masses
           case('NO','OFF','FALSE','F','no','off','false','f')
            if (qprint) then ; write(msg___,8001) whoami, 'WILL BE UNIFORM' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
            call ftsm_set_weights( (/ (1d0, i=1,natom) /), natom) ! uniform
           case('BFACTOR', 'bfactor', 'BFACT', 'bfact')
            if (qprint) then ; write(msg___,8001) whoami, 'SET FROM BFACTOR ARRAY' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
            call ftsm_set_weights(bfactor, natom) ! send masses
           case('OCCU', 'occu','OCCUPANCY', 'occupancy')
            if (qprint) then ; write(msg___,8001) whoami, 'SET FROM OCCUPANCY ARRAY' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
!*********************************************************************
     ...
     & ' WILL RESTRAIN SYSTEM TO PLANE PERPENDICULAR TO PATH.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
           case('NO','OFF','FALSE','F','no','off','false','f')
            proj_on=.false.
            if (qprint) then ; write (msg___,'(2A)') whoami, &
     & ' WILL RESTRAIN SYSTEM TO PATH IMAGE.'//                         &
     & ' (FE/MFPT CANNOT BE COMPUTED).' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
           case default
            call warning(whoami, 'UNKNOWN OPTION SPECIFIED', 0)
          end select
!*********************************************************************
...
! check replica spec
          ...
          if (irep.lt.0.or.irep.ge.nstring) then
           if (qprint) then ; write(msg___, 6773) whoami, whoami, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
           dpar0=zval
          else
           if (qprint) then ; write(msg___,6774) whoami, irep, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
! check replica spec
          irep=atoi(get_remove_parameter(comlyn, 'REP', comlen), -1)
          if (irep.lt.0.or.irep.ge.nstring) then
           if (qprint) then ; write(msg___, 6776) whoami, whoami, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
           dperp0=zval
          else
           if (qprint) then ; write(msg___,6775) whoami, irep, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
! check replica spec
          irep=atoi(get_remove_parameter(comlyn, 'REP', comlen), -1)
          if (irep.lt.0.or.irep.ge.nstring) then
           if (qprint) then ; write(msg___, 6777) whoami, whoami, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
           drms0=zval
          else
           if (qprint) then ; write(msg___,6778) whoami, irep, zval ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
! do a basic communicator check:
      if (ME_LOCAL.eq.0.and.ME_STRNG.eq.MPI_UNDEFINED) then
        write(msg___, 111) whoami, ME_GLOBAL, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 111 FORMAT(A, ' WORLD REPLICA ',I5, ' HAS ZERO GROUP ID', &
     ...
     & (ME_LOCAL.ne.0.or.MPI_COMM_LOCAL.eq.MPI_COMM_NULL)) then
        write(msg___, 112) whoami, ME_GLOBAL, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
      ...
      if (ftsm_initialized) then
       if (qroot) then
        if (ME_STRNG.eq.0) then
          write(msg___,'(2A)') &
     & whoami, ' FTSM ALREADY INITIALIZED. CALL "DONE" TO CLEAN UP.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='';
        endif ! ME_STRNG
       endif ! qroot
       return
      endif
...
!
     ...
     & whoami, ' FOUND ',nstring,' REPLICAS.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
      endif
!
...
!
      ...
      qprint=qroot.and.ME_STRNG.eq.0
!
      if (qprint) then ; write(msg___,8002) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
!
        if (qdiffrot) then
         ...
         do j=1, nforced;
          ...
          write(msg___,667) '\t',j, iatom_f(j), sid, rid, ren, ac ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         enddo
        else
         write(msg___,'(A)') '\t FORCING AND ORIENTATION ATOMS ARE THE SAME' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
      endif ! ME_STRING
!
 667 format(A,2I8,' ',4A)
...
!
     ...
     & whoami,' REDEFINING IMAGES (e.g. USING FILL)' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
      endif
 668 format(/2A,/2A)
!
...
!
! if (qslave) then
! call call mpi_bcast(r_f,9*nforced,MPI_INTEGER8,0,MPI_COMM_LOCAL,ierror) ! send three adjacent sets
! if (qdiffrot) call call mpi_bcast(r_o,9*norient,MPI_INTEGER8,0,MPI_COMM_LOCAL,ierror)
! endif ! qslave
!
...
!cccccccccccc print summary
       if (qprint) then
         if (rmsd0_flen.gt.0) then
          write(msg___,660 ) whoami,rmsd0_fname(1:rmsd0_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         else
          write(msg___,661 ) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
       endif
...
!ccccccccccc print summary
        if (qprint) then
         if (s_flen.gt.0) then
          write(msg___,652) whoami,s_fname(1:s_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         else
          write(msg___,653) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
        endif
...
!ccccccccccc print summary
        if (qprint) then
         if (c_flen.gt.0) then
          write(msg___,6521) whoami,c_fname(1:c_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         else
          write(msg___,6531) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
        endif
...
!ccccccccccc print summary cccccccccccccccccccccccccccccccccccccc
        if (qprint) then
         if (fe_flen.gt.0) then
          write(msg___,6520) whoami,fe_fname(1:fe_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         else
          write(msg___,6530) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
        endif
...
!cccccccccccc print summary
        if (centers_flen.gt.0) then
         output_centers=.true.
         if (qprint) then
          write(msg___,6620 ) whoami,centers_fname(1:centers_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
...
!
        if (rex_flen.gt.0) then
         output_rex_map=.true.
         if (qprint) then
          write(msg___,6721) whoami,rex_fname(1:rex_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
...
!
        if (rex_flen.gt.0) then
         output_rex_log=.true.
         if (qprint) then
          write(msg___,6723) whoami,whoami,rex_fname(1:rex_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
...
!ccccccccccc print summary
        if (forces_flen.gt.0) then
         output_forces=.true.
         if (qprint) then
          write(msg___,6625) whoami,forces_fname(1:forces_flen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         endif
       ...
       if (update_on.and.update_freq.gt.0) then
        if (mod(iteration,update_freq).eq.0) then
         if (.not.string_noprint) then
          write(msg___,'(2A)') whoami,' UPDATING STRING.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
         endif
!
         if (proj_on) then
...
! see if reparametrization requested
          if (repa_on) then
            if (.not.string_noprint) then
             write(msg___,'(2A)') whoami,' REPARAMETRIZING IMAGES.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
            endif
...
! update reference coordinates
          if (.not.string_noprint) then
           write(msg___,'(2A)') whoami,' UPDATING NEIGHBOR IMAGES.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
          endif
...
! save old reference coordinates and switch to new reference coordinates
          if (.not.string_noprint) then
           write(msg___,'(2A)') whoami,' UPDATING REFERENCE IMAGES.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
          endif
...
!
       if (repl_x_on.and.repl_x_freq.gt.0) then
        if (mod(iteration, repl_x_freq).eq.0) then
         if (.not.string_noprint) then
          write(msg___,'(2A)') whoami,' ATTEMPTING EXCHANGE OF NEIGHBORING REPLICAS.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
         endif
...
!
       if (stat_on.and.stat_freq.gt.0) then
         if (mod(iteration,stat_freq).eq.0) then
           write(msg___,'(2A)') whoami,' CALLING STRING STATISTICS.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
...
! call call mpi_bcast(fe,nstring,MPI_INTEGER8,0,MPI_COMM_LOCAL,ierror)
! endif
!
...
!
      ...
      if (qprint) then
       write(msg___,690) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),5);enddo;msg___=''
      endif

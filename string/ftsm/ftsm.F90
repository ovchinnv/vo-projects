...
! check fd spec
        ...
        if (qprint) then
         write(msg___, 7001) whoami,whoami,step,whoami,whoami
         do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! broadcast ifile so that all roots know whether handle is valid
         call mpi_bcast(ifile,1,MPI_INTEGER,0,MPI_COMM_STRNG,ierror)
        endif
...
! broadcast ifile so that all roots know whether handle is valid
         call mpi_bcast(ifile,1,MPI_INTEGER,0,MPI_COMM_STRNG,ierror)
         call mpi_bcast(ifile,1,MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
        endif
       endif ! qroot

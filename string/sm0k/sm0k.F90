...
!cccccccccccccccccc reparametrization option cccccccccccccccccccccc
     ...
     & whoami,' REPARAMETRIZATION WILL NOT BE DONE.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
       else
        repa_on=.true.
...
! if you want to follow the unparametrized dynamics, just set maxiter to 0 in the repa setup call
     ...
     & whoami,' STATISTICS WILL NOT BE OUTPUT.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
        else
         stat_on=.true.
         WRITE (msg___,'(/,2A,I6,A/)') &
     & whoami,' WILL OUTPUT STATISTICS AFTER EVERY ', &
     & stat_freq,' REPARAMETRIZATION ITERATIONS' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
      ...
      if (sm0k_initialized) then
       if (qroot) then
        if (ME_STRNG.eq.0) then
          write(msg___,'(2A)') &
     & whoami, ' SM0K ALREADY INITIALIZED. CALL "DONE" TO CLEAN UP.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
       endif
       return
      endif
...
! did the user specify filter cutoff?
       ...
       if (dst_cutoff.lt.0.0) then
        write(msg___,664) whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
...
!ccccccc CHECK FOR MULTIPLE OPTIONS
      if ((qspline+qlinear+qbspline+qdst+qlinear_exact) .eq. 0) then
       write(msg___,665) whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
...
mlen=min(max(0,mlen),len(methods(interp_method)));methods(interp_method)(mlen+1:)='';call adjustleft(methods(interp_method),(/' ',tab/));mlen=len_trim(methods(interp_method))
 ...
 667 format(A,' WILL REPARAMETRIZE STRING USING ',A,/, &
     &A,' INTERPOLATION TO WITHIN MAX(DS)/MIN(DS) < ',F7.3,' TOLERANCE')
      if (iterations.gt.0) then ; write(msg___,668) whoami, iterations ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___='' ; endif
 668 format(A,' WITH A MAXIMUM OF ',I3,' ITERATIONS')
      if(interp_method.eq.dst) then ; write(msg___,6680) whoami,dst_cutoff*100.0 ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___='' ; endif
 6680 format(A,' DST INTERPOLATION WILL USE THE LOWER ',F8.4, &
     & '% OF WAVENUMBERS')
!
      write(keyword,'(I8)') nmove ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
      mlen=len(keyword)
      mlen=min(max(0,mlen),len(keyword));keyword(mlen+1:)='';call adjustleft(keyword,(/' ',tab/));mlen=len_trim(keyword)
      write(msg___,669) whoami, keyword(1:mlen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
 669 format(A,' INTERPOLATION IS BASED ON ',A,' CARTESIAN COORDINATES')
      if (repa_mass.eq.1) then ; write(msg___,672) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___='' ; endif
 672 format(A,' INTERPOLATION WILL USE MASS-WEIGHTING')
!
!!!!!! orientation summary !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      if (orient.eq.1) then
        write(keyword,'(I8)') norient ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
        mlen=len(keyword)
        mlen=min(max(0,mlen),len(keyword));keyword(mlen+1:)='';call adjustleft(keyword,(/' ',tab/));mlen=len_trim(keyword)
        write(msg___,670) whoami, keyword(1:mlen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
 670 format(A,' STRING WILL BE ORIENTED BASED ON ',A,' ATOMS')
        if (orient_mass.eq.1) then ; write(msg___,671) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___='' ; endif
...
length=min(max(0,length),len(methods(interp_method)));methods(interp_method)(length+1:)='';call adjustleft(methods(interp_method),(/' ',tab/));length=len_trim(methods(interp_method))
        write(msg___,6770) whoami, methods(interp_method)(1:length) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6770 format(/A,' WILL INTERPOLATE USING ',A,' INTERPOLATION')
       else
        if (length.gt.0) then
         write(msg___,6771) whoami, method(1:length), whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6771 format(/A,' UNRECOGNIZED INTERPOLATION METHOD: ',A,'.',/, &
     & A, ' WILL INTERPOLATE USING LINEAR INTERPOLATION')
        else
         write(msg___,6772) whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! number of input replicas
 ...
 6781 format(A,' NUMBER OF INPUT REPLICAS MUST BE > 0. ',/, &
     & ' NOTHING DONE.')
        ...
        if (qprint) then
          write(msg___,6783) whoami, num_rep_in ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
 6783 format(A,' INITIAL STRING RESOLUTION: ', I5, ' REPLICAS.')
       endif ! num_rep_in<=0
      else
        if (qprint) then ; write(msg___, 6784) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
! number of output replicas
 ...
 6782 format(A,' NUMBER OF OUTPUT REPLICAS MUST BE > 0. ',/, &
     & ' NOTHING DONE.')
        ...
        if (qprint) then
          write(msg___,6785) whoami, num_rep_out ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
 6785 format(A,' INTERPOLATED STRING RESOLUTION: ', I5, ' REPLICAS.')
       endif ! num_rep_in<=0
      else
        if (qprint) then ; write(msg___, 6786) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
!
       do j=1, num_rep_in
        write(msg___,'(A1,I5," ",A80)') tab, j, fname_cor_in(j) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       enddo
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
!
       do j=1, num_rep_out
        write(msg___,'(A1,I5," ",A80)') tab, j, fname_cor_out(j) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       enddo
!
      endif ! qprint
!
...
!
       if (repa_mass.eq.1) then ; write(msg___,6720) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
! print a summary
        ...
        mlen=len(keyword)
        mlen=min(max(0,mlen),len(keyword));keyword(mlen+1:)='';call adjustleft(keyword,(/' ',tab/));mlen=len_trim(keyword)
        write(msg___,6700) whoami, keyword(1:mlen) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
 6700 format(A,' STRING WILL BE ORIENTED BASED ON ',A,' ATOMS')
        if (orient_mass.eq.1) then ; write(msg___,6710) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
 6710 format(A,' ORIENTATION WILL USE MASS-WEIGHTING')
       endif ! orient == 1
!ccccccccccccccccccccccccc do work cccccccccccccccccccccccccccc
       write(msg___,6974) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
...
! determine whether structures are to be oriented before comparison
       ...
       if (qstat_orient) then
        if (repa_initialized.and.norient.gt.0) then
         if (qprint) then ; write(msg___,638) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
...
!
        ...
        if (stat_rmsd_mass) then
         if (qprint) then ; write(msg___,640) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
         do i=1,nstat

...
! variables for cv and derivative calculations
       real*8, dimension(3) :: u, v, & ! the two vectors
     ...
     & fu, fv ! derivatives of angle w.r.t. vector components
       ...
       integer, allocatable, dimension(:) :: ind1, ind2, ind3, ind4, &
     & indf ! , indpsf1, indpsf2, indpsf3, indpsf4
       real*8, allocatable, dimension(:) :: x1, x2, x3, x4
       real*8, allocatable, dimension(:) :: y1, y2, y3, y4
       real*8, allocatable, dimension(:) :: z1, z2, z3, z4
       real*8, allocatable, dimension(:) :: m1, m2, m3, m4
...
! & ! vx, vy, vz, vn, ux, uy, uz, un,
     ...
     & f, w, totm1=0d0, totm2=0d0, totm3=0d0, totm4=0d0
       real*8 :: A(3,3), B(3,3) ! transformation matrices (frame vectors)
       real*8 :: dA(3,3) ! derivatives of transformation matrices
       logical :: qframe1, qframe2, qcom1, qcom2, qcom3, qcom4
...
!
        if (qframe2) then
         ...
         B=RESHAPE( (/1d0,0d0,0d0,0d0,1d0,0d0,0d0,0d0,1d0/),(/3,3/) ) ! for later use
        endif
!
...
!
       ...
       integer :: f1, f2, type
       integer :: j, k, ii, jj
       real*8, dimension(4,3) :: p ! points that define vectors
!
       data whoami /' CV_ANGLVEC_LIST>'/
!
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! first vector:
        write(msg___,'(A)') '\t ANGLE BETWEEN VECTORS, VECTOR 1' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        write(msg___,'(A,I5)') '\t COORDINATE FRAME: ',f1 ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
! first point:
        if (ncom1.eq.0) then ! constant point
         write(msg___,'(A,3(F11.5,", "))') '\t POINT 1, CONSTANT: ',p(1,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        else
         write(msg___,'(A)') '\t POINT 1, COM OF ATOM GROUP:' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         do j=1, ncom1;
...
! second point:
        if (ncom2.eq.0) then ! constant point
         write(msg___,'(A,3(F11.5,", "))') '\t POINT 2, CONSTANT: ',p(2,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        else
         write(msg___,'(A)') '\t POINT 2, COM OF ATOM GROUP:' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         do j=1, ncom2;
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! second vector:
        write(msg___,'(A)') '\t ANGLE BETWEEN VECTORS, VECTOR 2' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        write(msg___,'(A,I5)') '\t COORDINATE FRAME: ',f2 ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
! third point:
        if (ncom3.eq.0) then ! constant point
         write(msg___,'(A,3(F11.5,", "))') '\t POINT 3, CONSTANT: ',p(3,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        else
         write(msg___,'(A)') '\t POINT 3, COM OF ATOM GROUP:' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         do j=1, ncom3;
...
! fourth point:
        if (ncom4.eq.0) then ! constant point
         write(msg___,'(A,3(F11.5,", "))') '\t POINT 4, CONSTANT: ',p(4,:) ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        else
         write(msg___,'(A)') '\t POINT 4, COM OF ATOM GROUP:' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         do j=1, ncom4;

...
!
       real*8 :: x(:), y(:), z(:), mass(:)
       real*8 :: fx(:), fy(:), fz(:)
       real*8, optional :: fext ! `external' force ; present here for nominal compatibility, but (should) not be used
       integer :: i ! cv index
...
!
        ind=cv%priv(i)%p(2:ncv+1)
!
        write(msg___,'(A)') '\t CV RMSD FROM REFERENCE' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do jj=1, ncv;
         j=ind(jj)
         write(msg___,667) '\t INDEX: ',j ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        enddo
        deallocate(ind)
       endif
!

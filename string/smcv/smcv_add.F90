...
! z=sqrt( 1/N sum^N_i (z_i - z^0_i)^2 )
        ...
        write(msg___,*)'UNRECOGNIZED CV TYPE: ',keyword;call warning(whoami, msg___(1), 0)
      endif
!
...
! now process vector specifications (expecting four points)
       ...
       elseif (( key(1:2).eq.'P4'(1:2) )) then
        ...
        write(msg___,*)'VECTOR DEFINITION ERROR. EXPECTED "P[1-4]", GOT "',key,'"';call warning(whoami, msg___(1), 0)
       endif
!
...
! print short summary
      if (MPI_COMM_STRNG.ne.MPI_COMM_NULL.and.ME_STRNG.eq.0) then
       write(msg___, 665) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
      endif
!
...
!
       ...
       do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
!
       write(msg___,103) whoami, nforced ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       write(msg___,100) whoami, norient ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       if (qmass) then ; write(msg___,102) whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
       if (qtwo) then
        if (use_comp) then

...
!
         ...
         write(0,*) 'WARNING FROM: ',whoami,': ',' ERROR ADDING DIST_COM CV. NOTHING DONE.'
         cv_dist_com_add=.false.
        endif
       else ! found
         write(0,*) 'WARNING FROM: ',whoami,': ',' DIST_COM CV ALREADY PRESENT. NOTHING DONE.'
         cv_dist_com_add=.false.
       endif
...
! extract indices into the atom map
        ...
        ii=jj+1; jj=ii+ncom2-1; ind2=cv%priv(i)%p(ii:jj)
!
        write(msg___,'(A)') '\t DISTANCE-COM, GROUP 1' ; write(0,'(A)') msg___
        do j=1, ncom1;
        ...
        enddo
        write(msg___,'(A)') '\t DISTANCE-COM, GROUP 2' ; write(0,'(A)') msg___
        do j=1, ncom2;

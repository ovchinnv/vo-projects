...
! **********************************************************************!
!CHARMM Element source/stringm/cv_angle_com.mod $Revision: 1.0 $
!
! CV_ANGLE_COM.MOD
!
...
!
       ...
       real*8 :: dummy
!
       integer, allocatable, dimension(:) :: ind1, ind2, ind3
! & ,indpsf1, indpsf2, indpsf3
       real*8, allocatable, dimension(:) :: x1, x2, x3
       real*8, allocatable, dimension(:) :: y1, y2, y3
       real*8, allocatable, dimension(:) :: z1, z2, z3
       real*8, allocatable, dimension(:) :: m1, m2, m3
!
...
! extract indices into the atom map
        ...
        ii=jj+1; jj=ii+ncom3-1; ind3=cv%priv(i)%p(ii:jj)
!
        write(msg___,'(A)') '\t ANGLE-COM, GROUP 1' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom1;
        ...
        enddo
        write(msg___,'(A)') '\t ANGLE-COM, GROUP 2' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom2;
         iatom=cv%amap%i(ind2(j))
         sid=atoms%segid(iatom); rid=atoms%resid(iatom); ren=atoms%resname(iatom); ac=atoms%aname(iatom);
         write(msg___,667) '\t',j, iatom, sid, rid, ren, ac ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        enddo
        write(msg___,'(A)') '\t ANGLE-COM, GROUP 3' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom3;

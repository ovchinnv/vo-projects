...
! however, the parent routine (smcv_add) produces a unique ordering, so we don't check for this
       qdiffrot=1
...
! locals
       ...
       real*8 :: d, f, w, r1(3), r2(3), r_com(3), theta, u(3,3)
       integer, pointer :: priv(:)
       integer, pointer :: ind_o(:), ind_f(:)
       real*8, pointer :: r_o(:,:), rt_o(:,:), ow(:), &
     & r_f(:,:), rt_f(:,:), fw(:), rt_rot_f(:,:)
       real*8, dimension(:,:,:,:), pointer :: ugrad
!
...
!
       ...
       logical :: qdiffrot
       integer, allocatable, dimension(:) :: ind_o, ind_f
       data whoami /' CV_RMSD_LIST>'/
!
...
!
        allocate(ind_o(norient))
        ii=4; jj=ii+norient-1; ind_o=cv%priv(i)%p(ii:jj)
        write(msg___,'(A)') '\t RMSD FROM REFERENCE, ORIENTATION ATOMS' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, norient;
...
!
        if (qdiffrot) then
         allocate(ind_f(nforced))
         ii=jj+1; jj=ii+nforced-1; ind_f=cv%priv(i)%p(ii:jj)
         write(msg___,'(A)') '\t RMSD FROM REFERENCE, FORCED ATOMS' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
         do j=1, nforced;
         ...
         write(msg___,'(A)')'\t FORCED AND ORIENTATION ATOMS ARE THE SAME' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        endif
       endif ! ME_STRING
!
 667 format(A,2I8,' ',4A)

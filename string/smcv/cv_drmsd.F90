...
!
      real*8, parameter, public :: tol=1.0e-10
!
      contains
...
! locals
     ...
     & qdiffrot ! qdiffrot indicates that derivatives of rotation matrix need not be computed (oset=fset; ow=fw)
       ...
       real*8 :: u(3,3), oorho12 ! one over rmsd between target structures
...
!
 ...
 write(msg___,*)' ERROR ADDING',cvname,'CV. NOTHING DONE.';call warning(whoami, msg___(1), 0)
         cv_drmsd_add=.false.
        endif
       else ! found
 write(msg___,*)cvname,'CV ALREADY PRESENT. NOTHING DONE.';call warning(whoami, msg___(1), 0)
         cv_drmsd_add=.false.
       endif
!
...
! locals
       ...
       real*8 :: d, f, w, r1(3), r2(3), r_com(3), theta, u(3,3), u1(3,3)
       real*8 :: d1, rho, rho1, r11(3), r21(3), oorho12
       integer, pointer :: priv(:)
       integer, pointer :: ind_o(:), ind_f(:)
       real*8, pointer :: r_o(:,:), rt_o(:,:), rt1_o(:,:), ow(:), &
     & r_f(:,:), rt_f(:,:), rt1_f(:,:), fw(:), &
     & rt_rot_f(:,:), rt1_rot_f(:,:)
       real*8, dimension(:,:,:,:), pointer :: ugrad, ugrad1
!

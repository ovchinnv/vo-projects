...
!
      type cv_frame
      real*8, dimension(:,:), pointer :: o ! frame origin
      real*8, dimension(:,:,:), pointer :: r ! frame vectors
      real*8, dimension(:,:), pointer :: grado ! gradient vector of frame origin w.r.t. x y or z (the same) ;
      real*8, dimension(:,:,:,:,:), pointer :: gradr ! combined gradient array (with some extra space for `packed' communication)
      real*8, dimension(:,:,:,:), pointer :: gradrx ! gradient vector of frame vectors w.r.t. x ;
      real*8, dimension(:,:,:,:), pointer :: gradry !
      real*8, dimension(:,:,:,:), pointer :: gradrz !
...
!
       integer, allocatable, dimension(:) :: ind1
       real*8, allocatable, dimension(:) :: x1, y1, z1, m1
!
       integer :: ncom1, ind
       integer :: j, k, p, q, r, ii, jj
! variables for cv and derivative calculations
       real*8 :: xcom1, ycom1, zcom1, totm1
! real*8 :: C(3,3) ! mass-weighted correlation matrix
! real*8 :: invA(3,3), dum(3,3) ! auxiliary matrix
       real*8, dimension(3,3) :: C, A1, A2, A3, A4, invA ! 5.21.09: align frames the `correct' way
       real*8 :: a11, a12, a13, a21, a22, a23, a31, a32, a33
       real*8 :: detA, dist, w, corr1, corr2, corr3, corr4, cmax, dum
       real*8 :: mu(3), e ! eigenvalues
       real*8 :: b(3), v(3) ! auxiliary vector
       real*8 :: dVdC(3,3,3,3); ! derivative of axis( evector v w.r.t. correlation matrix Cij)
       real*8 :: dVdCfd(3,3,3,3); ! aardvark
       real*8, parameter :: tol=1e-8
       real*8 :: d_rp, d_pq, d_rq ! delta functions
!
...
! construct right hand side vector (b)
           do r=2,3
!ccccccccccccccccccc transfer function is NOT working properly!
! b(r-1)=v(q) * ( v(p)*v(r)-transfer(r.eq.p,0) ) + ! dC_pq contribution; note: transfer is a type cast from logical to integer
! & (1 - transfer(p.eq.q,0)) *
! & v(p) * ( v(q)*v(r)-transfer(r.eq.q,0) ) ! dC_qp contribution
             if (r.eq.p) then; d_rp=1d0; else; d_rp=0d0; endif
             if (p.eq.q) then; d_pq=1d0; else; d_pq=0d0; endif
             if (r.eq.q) then; d_rq=1d0; else; d_rq=0d0; endif
             b(r-1)=v(q) * ( v(p)*v(r)-d_rp ) + & ! dC_pq contribution
     & (1 - d_pq) * &
     & v(p) * ( v(q)*v(r)-d_rq ) ! dC_qp contribution
           enddo
           b(3)=0d0;
...
! character(len=80 fmt
       real*8 :: rtemp(3,3,frames%num_frames,SIZE_STRNG) ! temporary array
       integer :: ierror
...
! replica heads gather all data on root replica
     ...
     & 9*n,MPI_REAL, &
     & rtemp,9*n,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
       else
...
! scatter all data
        if (SIZE_STRNG.gt.1) then
         call MPI_SCATTER(rtemp,9*n,MPI_REAL, &
     & frames%r(:,:,1:n),9*n,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
        else
...
! locals
       ...
       real*8 :: U(3,3), A1(3,3), A2(3,3), A3(3,3), A4(3,3), B(3,3)
       real*8 :: corr1, corr2, corr3, corr4, cmax, wsum
!

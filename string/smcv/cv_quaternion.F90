...
!
      type cv_quat
      real*8, dimension(:,:), pointer :: q ! quaternion components
      real*8, dimension(:,:,:,:), pointer :: gradq ! combined gradient array with some `extra' space; parallelization efficiency
      real*8, dimension(:,:,:), pointer :: gradqx ! gradient vector of frame vectors w.r.t. x ;
      real*8, dimension(:,:,:), pointer :: gradqy !
      real*8, dimension(:,:,:), pointer :: gradqz !
...
!
      ...
      public quat_normalize ! normalize quaternion to unity
...
!
       if (ME_STRNG.eq.0) then
        ...
        f2=quat%priv(i)%p(2)
!
        write(msg___,'(A)') '\t ORIENTATION QUATERNION BETWEEN TWO FRAMES' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        write(msg___,'(A,I5)') '\t FRAME: ',f1 ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        write(msg___,'(A,I5)') '\t FRAME: ',f2 ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
       endif
!
...
!
       real*8 :: A(3,3), B(3,3), R(3,3) ! transformation matrices (frame vectors)
       real*8 :: dA(3,3) ! matrix derivatives
       real*8 :: qq(4) ! temporary quaternioin
       integer :: f1, f2, ii, j, jj, ncomf, ind
       integer, allocatable, dimension(:) :: indf
       real*8 :: a1, a2, trace, tr, dtr, s, s2, qdp
       logical :: qframe1, qframe2
       real*8 :: RRm32, RRm13, RRm21, RRp12, RRp13, RRp23
       integer, pointer :: priv(:)
!
       real*8, parameter :: tol=1e-2
...
! locals
       ...
       real*8 :: rtemp(4,quat%num_quat,SIZE_STRNG) ! temporary array
       integer :: ierror
       character(len=21) :: whoami
       data whoami /' QUAT_PRINT_GLOBAL>'/
!
       if (MPI_COMM_STRNG.eq.MPI_COMM_NULL) return
!
! do work
! gather all data on root
     ...
     & 4*n,MPI_REAL, &
     & rtemp,4*n,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
       else
        rtemp(:,:,1)=quat%q(:,1:n)
       endif
...
!
       ...
       data whoami /' QUAT_RESET_CALCULATE>'/
!
! check for initialization
       if (.not.quat_initialized) then

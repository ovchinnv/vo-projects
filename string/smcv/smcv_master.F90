...
!
      module smcv_master
      use cv_common
      use cv_frames, only: frames_reset_calculate
      use cv_quaternion, only: quat_reset_calculate
      use cv_types
      use sm_config
...
!
      ...
      public smcv_voronoi_smart_update ! experimental !
...
! locals
      ...
      logical :: qgrp
      integer :: i, ierror, qfac
!
      data whoami /' SMCV_MAIN>'/
...
! NOTE: this operation will destroy the current contents of the cv%k array
      if (planar_on.and..not.cv_common_k_initialized) then
       call warning(whoami, ' FORCE CONSTANTS FOR PLANAR SAMPLING UNDEFINED.', 0)
        write(msg___, 6765) whoami, whoami, whoami ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
...
!
     ...
     & ((iteration-unrestrained_eq0).lt.unrestrained_eq_steps))) then
!
        restraint_force_on=.true.
!
        if (restrained_eq_steps.gt.0) then
         s=min(1.0d0*(iteration-restrained_eq0)/restrained_eq_steps,1d0)
        else
         s=1d0
       ...
       if (evolve_cv_on.and.evolve_freq.gt.0) then
        if (mod(iteration,evolve_freq).eq.0) then ! evolve string
         if (.not.string_noprint) then
          write(msg___,'(2A)') whoami,' EVOLVING STRING.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
         endif
         ...
         elseif (evolve_aver_on) then ! just add the current theta values to the running average
           ...
           call cv_common_evolve_expo( 1d0-1d0/num_ave_cv_samples, &
     & evolve_nskip )
...
! compute M inverse
         ...
         if (unrestrained_on) then ! unrestrained dynamics
...
! write(6,*) 'ITERATION' ,iteration
         if (cv_common_voronoi_initialized) &
...
!
       if (repa_on.and.repa_freq.gt.0) then
         if (mod(iteration,repa_freq).eq.0) then
           if (.not.string_noprint) then
            write(msg___,'(2A)') whoami,' REPARAMETRIZING STRING.' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_),3);enddo;msg___=''
           endif
...
! (it is always added to history)
      ...
      real*8 :: u, v, t, t1, dot_pr
      logical :: deriv, calctheta, qgrp, qgrp1
      real*8, allocatable :: gradi(:), gradj(:)
      real*8 cvlist(cv%num_cv) ! for new M tensor computation
      integer :: cvlistsize, ii, jj
      character(len=15) :: whoami
      data whoami /' SMCV_ADD_HIST>'/
!
      qgrp=(MPI_COMM_LOCAL.ne.MPI_COMM_NULL) &
     & .and.(SIZE_LOCAL.gt.1)
!
...
!
! also need to add history for non-local indices (could also gather history on root before output, but isn't this faster?)
...
! this way, cvlistsize will be small, and the two inner loops will be fast
       cv%M(1:cv%num_cv,1:cv%num_cv,4)=0d0;
! do k=1, imap
       do k=kbeg, kend
        cvlistsize=cv%amap%v(k)%last
        cvlist=cv%amap%v(k)%i(1:cvlistsize) ! note that the cvs in the cvlist should be in increasing order because the map is updated as CV are added
        do j=1,cvlistsize
         jj=cvlist(j)
         do i=1, j
          ii=cvlist(i)
          cv%M(ii,jj,4)=cv%M(ii,jj,4)+ &
...
!
       do j=1,cv%num_cv
        do i=1,j
         cv%M(i,j,1)=t1*cv%M(i,j,1) + t*cv%M(i,j,4)
         cv%M(j,i,1)=cv%M(i,j,1) ! note: grad(:,:,2) is scaled by mass,
        enddo
       enddo
      endif
...
!
      real*8 :: r_send(1:cv%num_cv, 1:5) ! cv values to send to partner
      real*8 :: r_recv(1:cv%num_cv, 1:5) ! cv values to receive from partner
      real*8 :: dummy1, dummy2, dE_me, dE
!
...
!
      ...
      data whoami /' SMCV_REPL_EXCHANGE>'/
!
...
! 2) send/receive
        call MPI_SENDRECV(r_send, 5*cv%num_cv, MPI_REAL, &
     & which, which, r_recv, 5*cv%num_cv, MPI_REAL, &
...
! 4) combine energies from two replicas:
        call MPI_SENDRECV(dE_me, 1, MPI_REAL, &
     & which, which, dE, 1, MPI_REAL, &
     & which, ME_STRNG, MPI_COMM_STRNG, stat, ierror)
        dE=dE+dE_me
...
! may change this later
         if (which.lt.ME_STRNG) then
          success=(randomu().le.exp(-cv%rex_beta*dE))
...
! make entry in REX log (only lower-rank replica does this)
        if (ME_STRNG.lt.which) then
          i=int_vector_add(cv%rex_log, ME_STRNG) ! this replica
...
! call MPI_ALLGATHER(dE, 1, MPI_REAL,
! & dEG, 1, MPI_REAL, MPI_COMM_STRNG, ierror)
!
      endif ! MPI_COMM_STRNG
! from here on all nodes continue:
! broadcast success to all slave nodes
      if (qgrp) then
       call mpi_bcast(success,1,MPI_INTEGER4,0,MPI_COMM_LOCAL,ierror)
       call mpi_bcast(nodelist,nstring,MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
! broadcast new cv to slaves
       if (success) then
        call mpi_bcast(r_recv,cv%num_cv*5,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
       endif
      endif ! qgrp
!
! 6) if move was successful, perform actual exchange of CV
      if (success) then
        ...
        cv%r(1:cv%num_cv,comp)=r_recv(:,2) ! might comment this line so that restraint change gradually (shouldn't be necessary in principle!)
...
!
      ...
      real*8 :: dcv
      character(len=20) :: whoami
      data whoami /' SMCV_GRADCV_DOT_DR>'/
!
      if (.not.cv_common_grad_initialized) call cv_common_grad_init() ! allocate cv%grad array (for derivatives)
!
...
!
      ...
      data whoami /' SMCV_DCV_DOT_GRADCV>'/
!
      if (.not.cv_common_grad_initialized) call cv_common_grad_init() ! allocate cv%grad array (for derivatives)
!
      if (present(c)) then ; col=c ; else ; col=zold ; endif ! no bounds check so be careful
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 :: grad(cv%num_cv,cv%amap%last,3) ! temporary gradient array
      integer :: i, j, jj
      real*8 :: dummy
!
      character(len=19) :: whoami
      data whoami /' SMCV_TEST_GRAD_FD>'/
!
...
!
      ...
      logical :: qcv, qfr, qqt
      logical :: qgrp
      real*8 :: dummy(size(x,1))
!
      character(len=20) :: whoami
      data whoami /' SMCV_TEST_PARALLEL>'/
!
...
! locals
      ...
      integer :: i
      real*8 :: rtemp(cv%num_cv) ! for storing current CV values (theta(x))
!!
      character(len=23) :: whoami
      data whoami /' SMCV_VORONOI_WHEREAMI>'/
! do some work
!
...
!
      real*8 :: x(:), y(:), z(:)
      real*8 :: mass(:) ! assumed shape
! real*8 :: mass(size(x,1)) ! assumed size
...
! local var.
      ...
      real*8 :: rtemp(cv%num_cv) ! for storing current CV values (theta(x))
!
      integer, allocatable :: vtemp(:), vtemp2(:) ! for gathering Voronoi stats; this is an upper bound
      integer*4 :: stat(MPI_STATUS_SIZE)
      logical :: voronoi_update
      logical :: success, qgrp, qstring, ready(nstring-1), ok
      real*8 :: P_accept_cross
! for computing FE
! real*8 :: flux(nstring, nstring), ! normalized probability flux
...
!
     ...
     & MPI_INTEGER, 0, ME_STRNG, MPI_COMM_STRNG, request(ME_STRNG), ierror)
          else ! rank 0
           ...
           do while (any(.not.ready))
            do m=1,nstring-1
             if (.not.ready(m)) then
              ...
              if (ok) then
! get message length
                call MPI_Get_count(stat,MPI_INTEGER,length(m),ierror)
...
! begin receiving
     ...
     & MPI_INTEGER, m, m, MPI_COMM_STRNG, request(m), ierror)
           enddo ! m
...
! get message length
         call MPI_Get_count(stat,MPI_INTEGER,length(1),ierror)
         l=length(1)
! begin receiving
         allocate(vtemp2(l))
         call MPI_IRECV(vtemp2, l, MPI_INTEGER, &
...
!********************************************************************
         if (P_accept_cross.ge.1) then
          ...
          success=(randomu().le.P_accept_cross)
         endif
        endif ! string roots
! write(600+ME_STRNG,*) P_accept_cross, success, ' * ', prob
! broadcast to slaves
        if (qgrp) call mpi_bcast(success,1,MPI_INTEGER4,0,MPI_COMM_LOCAL,ierror)
...
! update voronoi log.
       i=int_vector_add(cv%voro_log, ME_STRNG+1) ! replica ID
...
! real*8 :: dx(:), dy(:), dz(:), xcomp(:), ycomp(:), zcomp(:)
      ...
      logical :: calcz, deriv, addforce
! local var.
      real*8 :: rnew(cv%num_cv) ! for storing current CV values (theta(x))
      real*8 :: rold(cv%num_cv) ! for storing additional (old) CV values (theta(xcomp))
      real*8 :: A(3,3,frames%num_frames), o(3,frames%num_frames), &
     & q(4,quat%num_quat)
...
! local
      ...
      character(len=11) :: whoami
      logical :: qprint
      data whoami /' SMCV_LIST>'/
!
      qprint=MPI_COMM_STRNG.ne.MPI_COMM_NULL &
     & .and.ME_STRNG.eq.0
!
      do i=1, cv%num_cv
       if (qprint) then ; write(msg___,666) whoami,i ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___='' ; endif
 666 format(A,' CV # ',I8,':')
       select case (cv%type(i))
         case (posi_com_x, posi_com_y, posi_com_z);
...
! compute M inverse; assume that M is regular
...
!
      ...
      real*8 :: diff
      real*8 :: temp(cv%num_cv, cv%num_cv)
      logical :: qlu
      qlu=inverse_LU
      inverse_LU=.true.
...
! & cv%amap%last, MPI_REAL, frames%o,
! & fr_send_count*cv%amap%last, fr_send_displ*cv%amap%last,
! & MPI_REAL, MPI_COMM_LOCAL, ierror)
        call mpi_allgatherv(frames%gradr(1,1,1,1,ibeg), &
     & fr_send_count(ME_LOCAL+1)*ffac, MPI_REAL, &
...
! communicate
     ...
     & qt_send_count(ME_LOCAL+1)*qfac, MPI_REAL, &

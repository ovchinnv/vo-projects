...
! extract indices into the atom map
        ...
        ii=jj+1; jj=ii+ncom4-1; ind4=cv%priv(i)%p(ii:jj)
!
        write(msg___,'(A)') '\t DIHEDRAL-COM, GROUP 1' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom1;
        ...
        enddo
        write(msg___,'(A)') '\t DIHEDRAL-COM, GROUP 2' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom2;
         iatom=cv%amap%i(ind2(j))
         sid=atoms%segid(iatom); rid=atoms%resid(iatom); ren=atoms%resname(iatom); ac=atoms%aname(iatom);
         write(msg___,667) '\t',j, iatom, sid, rid, ren, ac ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        enddo
        write(msg___,'(A)') '\t DIHEDRAL-COM, GROUP 3' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom3;
         iatom=cv%amap%i(ind3(j))
         sid=atoms%segid(iatom); rid=atoms%resid(iatom); ren=atoms%resname(iatom); ac=atoms%aname(iatom);
         write(msg___,667) '\t',j, iatom, sid, rid, ren, ac ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        enddo
        write(msg___,'(A)') '\t DIHEDRAL-COM, GROUP 4' ; do i_=1,size(msg___);if(msg___(i_)=='')exit;call plainmessage(msg___(i_));enddo;msg___=''
        do j=1, ncom4;

...
!****************************************************************************************************************************
      type cv_base
!
      real*8, dimension(:,:), pointer :: r ! cv coordinates
      real*8, dimension(:,:,:), pointer :: M ! mass matrix: (running average); two slots for M, (:,:,1) and (:,:,2); two slots for M^{-1} (:,:,3-4)
      real*8, dimension(:,:,:,:), pointer :: grad ! combined gradient array; introduced for parallelization (comm. efficiency)
      real*8, dimension(:,:,:), pointer :: gradx ! gradient vector of CV w.r.t. x ; (:,:,2) is mass-weighted to compute M
      real*8, dimension(:,:,:), pointer :: grady ! gradient vector of CV w.r.t. y ; (:,:,2) is mass-weighted to compute M
      real*8, dimension(:,:,:), pointer :: gradz ! gradient vector of CV w.r.t. z ; (:,:,2) is mass-weighted to compute M
...
! that are "inactive" in reparametrization, df computation, etc.; currently used for SMD
      ...
      real*8, dimension(:), pointer :: r_bc_0, r_bc_1 ! arrays for optional fixed b.c.
      real*8, dimension(:), pointer :: k ! force constant
      real*8 :: kpara ! force constant for forces parallel to string ; same for all CV (required for off-path sim.)
      real*8 :: kperp ! force constant for forces perpendicular to string ; same for all CV
      real*8, dimension(:), pointer :: gamma ! coupling constant for on-the-fly string (currently not used)
      real*8, dimension(:), pointer :: weight
      real*8 :: wrss ! one over root-sum-square of the weight array
      ...
      real*8 :: dt=0 ! timestep for string evolution
      ...
      real*8, dimension(:), pointer :: ds, fe, curvature, feav ! arclength,FE,curvature, avg. FE
      real*8, dimension(:,:), pointer :: rall ! holds a complete set of CV for Voronoi T.
      real*8, dimension(:,:,:), pointer :: Mall ! holds a complete set of M matrices & inverses (Voronoi T.)
      real*8 :: work ! nonequilibrium work
...
!
      ...
      real*8 :: voronoi_cut ! voronoi cell cutoff in path-perpendicular direction (beyond which MD replicas are not allowed)
      ...
      real*8 :: rex_beta ! ensemble temperature in replica exchange (usually will be the same as the simulation temperature)
      end type cv_base
      ! subroutines
...
!
      ! parameters
      integer, parameter, public :: max_cv_common=1000,max_hist_base=10000
...
! assign cv values
        cv%num_cv=j
        cv%r(j,:)=unknownf ! undefined value
!
        cv%k(j)=0d0
        if (present(k)) then
...
!
       logical :: qroot
!
       real*8 :: Mave(max_cv_common, cv%num_cv)
...
!
     ...
     & MPI_REAL, MPI_SUM, MPI_COMM_STRNG, ierror) ! use M inverse averaged along string
...
!
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) then
        call mpi_bcast(cv%weight(1:cv%num_cv),cv%num_cv,MPI_REAL,0,MPI_COMM_LOCAL,ierror) ! broadcast within groups
       endif
!
...
!
         ...
         real*8, optional :: r_bc_0(n), r_bc_1(n)
...
! send to slaves
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) then
        call mpi_bcast(cv%r(1:cv%num_cv,main),cv%num_cv,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
        call mpi_bcast(cv%r(1:cv%num_cv,dz),cv%num_cv,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
        call mpi_bcast(cv%ds,nstring-1,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
        call mpi_bcast(cv%curvature,nstring-2,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
       endif
...
! locals
       ...
       real*8 :: t(max_hist_base)=(/(1.0d0*i,i=1,max_hist_base)/)
       real*8 :: temp(max_hist_base)=0.0d0, u ! for smoothing
       character(len=21) :: whoami
...
!
        integer :: delta
        real*8 :: t(:), x(:)
        real*8, dimension(size(t)) :: smooth2
...
! gather all data on root (0)
        call MPI_GATHER(rtemp,cv%num_cv*kout,MPI_REAL, &
     & rall,cv%num_cv*kout,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
!
...
! locals
       ...
       real*8 :: temp(max_hist_base)=0.0d0
!
! do work
...
! locals
       ...
       logical :: bc
       character(len=80) :: fmt
       real*8, allocatable :: rtemp(:,:) ! temporary array for holding coords
       integer :: ierror
       logical :: qroot
...
!
        call MPI_GATHER(cv%r(1,c),cv%num_cv,MPI_REAL, &
     & rtemp,cv%num_cv,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
!
        if (ME_STRNG.eq.0) then ! root writes
!
...
! gather all data on each root processor
     ...
     & call MPI_ALLGATHER(cv%r(1,c),cv%num_cv,MPI_REAL, &
     & rtemp(1,1+cv_common_fixed_0_bc), &
     & cv%num_cv,MPI_REAL,MPI_COMM_STRNG, &
     & ierror)
! broadcast rtemp to slaves:
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) &
     & call mpi_bcast(rtemp,cv%num_cv*nrep,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
!
...
! send to slaves
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) then
        call mpi_bcast(cv%r(1:cv%num_cv,forces),cv%num_cv,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
        call mpi_bcast(cv%fe,nstring,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
       endif
!
...
!
       ...
       integer :: i, k, c
       real*8 :: rtemp(max_cv_common, nstring), dummy ! temporary array for holding coords
       integer :: ierror
...
! scatter all data on root
       if (MPI_COMM_STRNG.ne.MPI_COMM_NULL) then
        call MPI_SCATTER(rtemp, max_cv_common, MPI_REAL, &
     & cv%r(1,c),max_cv_common,MPI_REAL,0,MPI_COMM_STRNG, &
     & ierror)
! broadcast BC
        if (bc) then
         if (cv_common_fixed_0_bc.eq.1) &
     & call mpi_bcast(cv%r_bc_0,cv%num_cv,MPI_REAL,0,MPI_COMM_STRNG,ierror)
...
! check for zero coordinate entries
      ...
      write(msg___,*)'SOME CV VALUES ARE UNDEFINED AFTER READING ON REPLICA',me,'.';call warning(whoami, msg___(1), 0)
        endif
...
!
       ...
       integer, optional :: col
       integer :: i, k, c
       real*8 :: rtemp(max_cv_common, numrep) ! temporary array for holding coords
       integer :: ierror
...
! now broadcast CV values to other nodes
       if (qroot) then
        if (SIZE_STRNG.gt.1) &
     & call mpi_bcast(rtemp,max_cv_common*numrep,MPI_REAL,0,MPI_COMM_STRNG,ierror)
...
write(msg___,*)'REPLICA',me,'ATTEMPTED TO READ NONEXISTENT COLUMN',which,'. NOTHING DONE.';call warning(whoami, msg___(1), 0)
        endif ! replica
! check for zero coordinate entries
        if (any(cv%r(1:cv%num_cv,c).eq.0.0d0)) then
         ...
         i=len(me)
         i=min(max(0,i),len(me));me(i+1:)='';call adjustleft(me,(/' ',tab/));i=len_trim(me)
write(msg___,*)'SOME CV COORDINATES ARE ZERO AFTER READING ON REPLICA ',me,'.';call warning(whoami, msg___(1), 0)
        endif
       endif ! root
!
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       integer :: c1, c2
! local
       real*8 :: cv_common_rmsd, dummy, dangle
! real*8, parameter :: pi=3.14159265358979 ! hardwired for angles
       integer :: i
       character(len=14) :: whoami
...
!
       if (.not.cv_common_rex_initialized) then
        ...
        if (t.gt.0) cv%rex_beta=1d0/(t*kboltzmann)
...
! & 0,MPI_COMM_LOCAL,ierr)
     & call mpi_bcast(cv%rex_map,nstring,MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
!
...
! calculate send displacements
        ...
        enddo
! now gather the logs
        call MPI_ALLGATHERV(cv%rex_log%i,cv%rex_log%last,MPI_INTEGER, &
     & rex_log_all,rex_log_size4,rex_log_disp4,MPI_INTEGER, &
     & MPI_COMM_STRNG,ierror)
!
        if (.not.present(fmt)) then
         frm='(2I5,I8)'
...
! & cv%voronoi_map, 1, MPI_INTEGER, MPI_COMM_STRNG, ierr)
!
        if (ME_STRNG.eq.0) then
         if (.not.present(fmt)) then
          write(frm,'("(",I5,"I5)")') nstring
...
! gather all main coordinate sets (same code as in print_global)
       if (MPI_COMM_STRNG.ne.MPI_COMM_NULL.and.SIZE_STRNG.gt.1) then
        call MPI_ALLGATHER(cv%r,cv%num_cv,MPI_REAL, &
     & cv%rall,cv%num_cv,MPI_REAL,MPI_COMM_STRNG, &
     & ierror)
...
!
     ...
     & cv%num_cv**2,MPI_REAL, &
     & cv%Mall,cv%num_cv**2,MPI_REAL,MPI_COMM_STRNG, &
     & ierror)
       endif
!
! broadcast to slaves
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) then
        call mpi_bcast(cv%rall,cv%num_cv*nstring,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
        call mpi_bcast(cv%Mall,(cv%num_cv**2)*nstring,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
       endif
!
       cv_common_voronoi_wrong_cell=.false.
...
!
       ...
       real*8 :: rold(cv%num_cv) ! new reflected CV values (theta(xcomp))
       integer, optional :: m_iter
! locals
       real*8, pointer :: M(:,:)
       real*8 :: rall_new(cv%num_cv, nstring)
       real*8 :: rall_temp(cv%num_cv, nstring)
       real*8 :: Mall_new(cv%num_cv, cv%num_cv, nstring)
       real*8 :: Mall_temp(cv%num_cv, cv%num_cv, nstring)
       real*8 :: z(cv%num_cv)
       integer :: correct_cell_me, correct_cell
       integer :: iter, max_iter
       integer, parameter :: default_iter=15
       real*8 :: dummy
       integer :: i, j, which(1), ierror, me
...
! gather all main cv coordinate sets
         call MPI_ALLGATHER(cv%r,cv%num_cv,MPI_REAL, &
     & rall_new,cv%num_cv,MPI_REAL,MPI_COMM_STRNG, &
     & ierror)
! gather all M^-1 matrices
...
!
     ...
     & cv%num_cv**2,MPI_REAL, &
     & Mall_new,cv%num_cv**2,MPI_REAL,MPI_COMM_STRNG, &
     & ierror)
       endif
!
       if (.not.cv_common_voronoi_initialized) then
...
! locals
       integer :: i,ii,j,jj,k
       real*8 :: z(ncv), dummy, msd_n(nstring), msd_m(nstring)
       logical :: q
       do i=1,nstring
!
...
! locals
       real*8 :: msd(nstring,nstring), msdall(nstring,nstring) ! mean square distance
       real*8 :: dummy, z(cv%num_cv)
       integer :: i, j, k, n, m, iatom, which(1), q(nstring), me
! parallelization
       logical :: qpara
...
!
       if (qpara) &
        call mpi_bcast(which,1,MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
...
! do work
! gather all data on root
       type=MPI_INTEGER !!**CHARMM_ONLY**!##.not.INTEGER8
       type=MPI_INTEGER8 !!**CHARMM_ONLY**!##INTEGER8
       if (.not.cv_common_voronoi_initialized) call cv_common_voronoi_init()
!
       if (MPI_COMM_STRNG.ne.MPI_COMM_NULL) then
...
!
        if (SIZE_STRNG.gt.1) then
         call mpi_bcast(cv%voronoi_data(:,:,3),SIZE_STRNG*(2*SIZE_STRNG+1),MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
        endif
       endif ! MPI_COMM_STRNG
! broadcast to slaves
       if (MPI_COMM_LOCAL.ne.MPI_COMM_NULL.and.SIZE_LOCAL.gt.1) &
     & call mpi_bcast(cv%voronoi_data(:,:,3),nstring*(2*nstring+1),MPI_INTEGER,0,MPI_COMM_LOCAL,ierror)
!
...
! & 0,MPI_COMM_WORLD,ierror)
        call MPI_ALLGATHER(cv%voro_log%last,1,MPI_INTEGER, &
     & voro_log_size8,1,MPI_INTEGER, &
     & MPI_COMM_STRNG,ierror)
...
! & 0,MPI_COMM_WORLD,ierror)
        call MPI_ALLGATHERV(cv%voro_log%i,cv%voro_log%last,MPI_INTEGER, &
     & voro_log_all,voro_log_size4,voro_log_disp4,MPI_INTEGER,&
     & MPI_COMM_STRNG,ierror)
!
...
!
        ...
        call MPI_SCATTER(M_all,cv%num_cv**2,MPI_REAL, &
     & M_me,cv%num_cv**2,MPI_REAL,0, &
     & MPI_COMM_STRNG,ierror)
       endif ! COMM_STRNG
! broadcast to slaves
       if (ME_LOCAL.ne.MPI_UNDEFINED.and.SIZE_LOCAL.gt.1) &
     & call mpi_bcast(M_me,cv%num_cv**2,MPI_REAL,0,MPI_COMM_LOCAL,ierror)
!
...
!
        call MPI_GATHER(M_me,isizeM,MPI_REAL, &
     & M_all,isizeM,MPI_REAL,0, &
     & MPI_COMM_STRNG,ierror)
! call MPI_ALLGATHER(M_me,isizeM,MPI_REAL,
! & M_all,isizeM,MPI_REAL,
! & MPI_COMM_STRNG,ierror)
...
! local declarations
     ...
     & dr(max_cv_common,num_rep_in-1)
       real*8 :: rout(max_cv_common,num_rep_out)
       real*8 :: rr(num_rep_in), rr_out(num_rep_out), ds(num_rep_in-1), &

...
! check for duplicate CV (exact identical entry only)
    ...
    write(msg___,*)' QUAT_',pos,'CV ALREADY PRESENT. NOTHING DONE.';call warning(whoami, msg___(1), 0)
            cv_qcomp_add=.false.
            return
         endif
        endif
...
!****************************************************************************************
        ...
        write(msg___,*)' ERROR ADDING QCOMP_',pos,'CV. NOTHING DONE.';call warning(whoami, msg___(1), 0)
        cv_qcomp_add=.false.
       endif
...
!
     ...
     & fx(:), fy(:), fz(:), mass(:)
       ...
       logical :: calctheta ! whether or not to calculate theta(x); if not, use theta=cv%r(i,instant)
                            ! note that if calctheta=.false., we don't calculate the derivatives either

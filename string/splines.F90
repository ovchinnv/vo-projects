...
! **********************************************************************!
!CHARMM Element source/machdep/splines.src 1.1
!
!
...
! locals
      integer :: nt, nx, i
      real*8, dimension(size(t)) :: xtemp, xnew, smooth2
!
...
write(msg___,*)'FILTER WIDTH TOO LARGE. SETTING TO ',delta,'.';call warning('SMOOTH2', msg___(1), 0)
!
      elseif (delta.le.0) then
       delta=max(1,floor(1.0d0*nx/3.0d0))
write(msg___,*)'FILTER WIDTH ZERO. SETTING TO ',delta,'.';call warning('SMOOTH2', msg___(1), 0)
      endif
!
      do i=1, delta

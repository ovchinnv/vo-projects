...
// compute plugin forces
  ierr=sm_dyna_from_acemd(iteration, r, fr, &sm_energy, &atomlist); // might return valid atomlist
...
//
  ierr=sm_dyna_from_acemd(iteration, r, fr, &sm_energy, &atomlist); // atomlist should not be modified in this call
//
  for (aptr=atomlist+1 ; aptr<atomlist + 1 + (*atomlist) ; aptr++) { // iterate until atomlist points to the last index
   j=*aptr - 1;
   fptr=fr + 3*j ;

...
//registers
 float w, e, s, n, b, f, d ;// FD coefficients
//
...
// center metric ; NOTE that the center metric is loarger, of size 2*_BSIZE_X+1 ; we read two blocks of size _BSIZE_X from each end ; skip one in the middle; then compute it
...
// NOTE : it might be better to prefetch z metrics into a shmem array;
 if ( !( (bool)ty || (bool)tx ) ) { // first thread reads
  for (int i=1 ; i < _SX ; i++) { // should work for any _SX ; could be slow ; 
   dxcen[i*_BSIZE_Y] = 0.5 * ( dxcor[i*_BSIZE_X]+dxcor[i*_BSIZE_X+1] );
  }
  for (int j=1 ; j < _SY ; j++) {
...
// ...
  if (ty < 4) {
   p  [IOL(tx+1+_BSIZE_X*(ty%2), ty+(ty/2)*(_BSIZE_Y+1))] = devp  [IOG(ix+1+_BSIZE_X*(ty%2), iy+(ty/2)*(_BSIZE_Y+1), k)];
   eps[IOL(tx+1+_BSIZE_X*(ty%2), ty+(ty/2)*(_BSIZE_Y+1))] = deveps[IOG(ix+1+_BSIZE_X*(ty%2), iy+(ty/2)*(_BSIZE_Y+1), k)];
  }
  if (tx < 2) {
   p  [IOL(tx+(_SX*_BSIZE_X+1)*(tx%2), ty+1)] = devp  [IOG(ix+(_SX*_BSIZE_X+1)*(tx%2), iy+1, k)];
   eps[IOL(tx+(_SX*_BSIZE_X+1)*(tx%2), ty+1)] = deveps[IOG(ix+(_SX*_BSIZE_X+1)*(tx%2), iy+1, k)];
  }
...
// (1) red
  ...
  b = ( ebot[0] + ecur[0] )              / ( dzcenbot * dzcor );
  f = ( etop[0] + ecur[0] )              / ( dzcentop * dzcor );
  ...
  b = ( ebot[1] + ecur[1] )              / ( dzcenbot * dzcor );
  f = ( etop[1] + ecur[1] )              / ( dzcentop * dzcor );

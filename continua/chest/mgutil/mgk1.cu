...
// registers 15 + 2=17
 int ix = _SX*(blockIdx.x * _BGSMAX_X) + tx;
 int iy = _SY*(blockIdx.y * _BGSMAX_Y) + ty;
//registers 17 + 7 = 22
...
//local memory:
#define IOL(i,j)  ((j)*(_SX*_BGSMAX_X+2) + (i))
#define IIL(i,j)  ((j)*(_SX*_BGSMAX_X)   + (i))
...
// x-metrics
 if (ty < _SX) {
// center metric ; NOTE that the center metric is larger, of size 2*_BGSMAX_X+1 ; we read two blocks of size _BGSMAX_X from each end ; skip one in the middle; then compute it
//  dxcen[ix]         = devdx[i1 - 1 + ix] ; // first half of tile
//  dxcen[ix+_BGSMAX+1]= devdx[i1 - 1 + ix +_BGSMAX+1] ; // second half of tile
  oodxcen[tx+_BGSMAX_X*ty+(ty>0) ] = devoodx[i1 - 1 + ix + _BGSMAX_X*ty+(ty>0)];
// corner metric
//  dxcor[ix] = devdx[i1 - 1 + nx-1 + ix ];                   // first  x-half of tile
//  dxcor[ix+_BGSMAX_X] = devdx[i1 - 1 + nx-1 + ix +_BGSMAX_X]; // second x-half of tile
// in one shot, assuming threIdx.y<2 :
  oodxcor[tx+_BGSMAX_X*ty] = devoodx[i1 - 1 + (nx-1) + ix + _BGSMAX_X*ty];
 }
// NOTE that if we know that tx=ty we can use a single if clause to read both metrics
// y-metrics
 if (tx < _SY) {
// center
  oodycen[ty+_BGSMAX_Y*tx+(tx>0)] = devoody[j1 - 1 + iy + _BGSMAX_Y*tx+(tx>0)];
// corner
  oodycor[ty+_BGSMAX_Y*tx]     = devoody[j1 - 1 + ny-1 + iy + _BGSMAX_Y*tx];
 }
//
...
// deal with y-metric first, since it will usually require global mem read.
  if (_SY<2) { // read from gmem
   oodycen[_BGSMAX_Y] = devoody[j1 - 1 + (iy - ty) + _BGSMAX_Y];
  } else {
   oodycen[_BGSMAX_Y] = 2.0 / ( 1.0/oodycor[_BGSMAX_Y-1]+1.0/oodycor[_BGSMAX_Y-(_SY<2)] ); // ( _SY<2 ) avoids static memory check warnings
  }
//
//  if (_SX<2) {  // read from gmem
   oodxcen[_BGSMAX_X] = devoodx[i1 - 1 + (ix - tx) + _BGSMAX_X];
//  } else {
//   oodxcen[_BGSMAX_X] = 2.0 / ( 1.0/oodxcor[_BGSMAX_X-1]+1.0/oodxcor[_BGSMAX_X-(_SX<2)] );
//  }
 }
//
...
//   p[IOL(tx+1+_BGSMAX_X,ty)] = devp[IOG(ix+1+_BGSMAX_X,iy,k); // second
// far boundary
//   p[IOL(tx+1,ty+_BGSMAX_Y+1)]          = devp[IOG(ix+1,         iy+_BGSMAX_Y+1,k); // red
//   p[IOL(tx+1+_BGSMAX_X,ty+_BGSMAX_Y+1)] = devp[IOG(ix+1+_BGSMAX_X,iy+_BGSMAX_Y+1,k); // black
//
// epsilon
//   eps[IOL(tx+1,         ty)] = deveps[IOG(ix+1,         iy,k); // first block
//   eps[IOL(tx+1+_BGSMAX_X,ty)] = deveps[IOG(ix+1+_BGSMAX_X,iy,k); // second
//
//   eps[IOL(tx+1,         ty+_BGSMAX_Y+1)] = deveps[IOG(ix+1,         iy+_BGSMAX_Y+1,k); // red
//   eps[IOL(tx+1+_BGSMAX_X,ty+_BGSMAX_Y+1)] = deveps[IOG(ix+1+_BGSMAX_X,iy+_BGSMAX_Y+1,k); // black
//  }
// same for y-rows -- omitted here
// ...
  if (ty < 4) { // BC in y direction
   p  [IOL(tx+1+_BGSMAX_X*(ty%2), (ty/2)*(_BGSMAX_Y+1))] = devp  [IOG(ix+1+_BGSMAX_X*(ty%2), iy-ty+(ty/2)*(_BGSMAX_Y+1), k)]; // subtract ty to get first tile coordinate
   eps[IOL(tx+1+_BGSMAX_X*(ty%2), (ty/2)*(_BGSMAX_Y+1))] = deveps[IOG(ix+1+_BGSMAX_X*(ty%2), iy-ty+(ty/2)*(_BGSMAX_Y+1), k)];
  }
  if (tx < 2) {
   p  [IOL((_SX*_BGSMAX_X+1)*(tx%2), ty+1)] = devp  [IOG(ix-tx+(_SX*_BGSMAX_X+1)*(tx%2), iy+1, k)];
   eps[IOL((_SX*_BGSMAX_X+1)*(tx%2), ty+1)] = deveps[IOG(ix-tx+(_SX*_BGSMAX_X+1)*(tx%2), iy+1, k)];
  }
...
//  rhs [ IIL(2*tx+1,ty) ] = devrhs [ IIG(ix+1,iy,k-1) ];
  ...
  rhs   [ IIL(tx+_BGSMAX_X,ty) ] = devrhs   [ IIG(ix+_BGSMAX_X,iy,k-1) ];
//
  kappa [ IIL(tx,         ty) ] = devkappa [ IIG(ix,         iy,k-1) ];
  kappa [ IIL(tx+_BGSMAX_X,ty) ] = devkappa [ IIG(ix+_BGSMAX_X,iy,k-1) ];
...
//  if (qred){
  ...
  s = ( eps[indl-(_SX*_BGSMAX_X+2)] + ecur[_DX] ) * ( oodycor[ty] * oodycen[ty] );
  n = ( eps[indl+(_SX*_BGSMAX_X+2)] + ecur[_DX] ) * ( oodycor[ty] * oodycen[ty+1] );
...
//  pcur0 -= dt * ( pcur0 + o * ( w*p[IOL(2*tx,ty+1)] + e*p[IOL(2*tx+2,ty+1)] + s*p[IOL(2*tx+1,ty)] + n*p[IOL(2*tx+1,ty+2)] + f*pfront[0] + b*pback0 - 2.0f*rhs[IIL(2*tx,ty)] ) );
  pcur[_DX] -= dt * ( pcur[_DX] + o * ( w*p[indl-1] + e*p[indl+1] + s*p[indl-(_SX*_BGSMAX_X+2)] + n*p[indl+(_SX*_BGSMAX_X+2)] + f*pfront[_DX] + b*pback[_DX]

  ...
  module bestfit
     ...
     & RESHAPE( (/one,zero,zero,zero,one,zero,zero,zero,one/), (/3,3/) ) ! Kronecker delta
      ...
      !###############################################################
       ...
       if (norm3.lt.ERRTOL()*50) norm3=zero
       end function norm3
      !###############################################################
       function unit3(v)
       implicit none
       real*8 :: v(3), norm, unit3(3)
      ! need to be careful with roundoff error
       norm=sqrt(v(1)*v(1)+v(2)*v(2)+v(3)*v(3))
       if (norm.lt.ERRTOL()*50) then; norm=one; else; norm=one/norm; endif
       unit3=(/v(1)*norm, v(2)*norm, v(3)*norm/)
       end function unit3
      !###############################################################
       function veccross3(v1,v2)
       implicit none
       real*8 :: v1(3), v2(3), veccross3(3)
...
! write(6,*) '*******RMSD: ',rmsd
      ...
      !
       ...
       real*8 :: a0, a1, a2, Qq, Rr, Dd, rootQ, rootQ3, theta
       real*8 :: a,b,c,d,e,f,g,h,i,det13,det12
       real*8 :: eval(3), evec(3,3), p(3), q(3), r(3), v(3), &
     & dummy, pn, qn
       integer :: j,k,l
      !
       real*8:: errtol_
     ...
     & a11*a23*a32-a13*a31*a22-a12*a21*a33)
      ...
      !
         ...
         if (abs(b) .gt. errtol_) then; v=unit3((/one, -a/b, zero/)) !;write(0,*) 216, b
         elseif (abs(e) .gt. errtol_) then; v=unit3((/one, -d/e, zero/)) !;write(0,*) 217, e
         elseif (abs(h) .gt. errtol_) then; v=unit3((/one, -g/h, zero/)) !;write(0,*) 218, h
         else ! Q=0 this should not be possible because P Q R are checked above !
           v=(/zero, one, zero/) !;write(0,*) 220
         endif
        endif
        evec(:,j)=v
...
!
      ! note: not requiring that sum(w)=1, but this is HIGHLY recommended for FP precision
       real*8 :: x0(:,:), y0(:,:), w(:)
       real*8 :: u(3,3)
       integer, optional :: ibeg, iend
       real*8, optional :: gradu(:,:,:,:) ! gradient of rotation matrix wrt x0
       real*8, optional, intent(out) :: eigval(3) ! eigenvalues
       real*8, dimension(size(x0,1)) :: x1, x2, x3, y1, y2, y3
       real*8 :: xcom(3),ycom(3),r11,r12,r13,r21,r22,r23,r31,r32,r33, &
...
! for derivatives:
       ...
       real*8 :: gradb(3,3,3)
       real*8 :: dvdrrfd(3,3,3,3), dmudrrfd(3,3,3), dum(3,3) ! FD
       real*8 :: a11, a12, a13, a21, a22, a23, a31, a32, a33
       integer :: p, q, s
       real*8, allocatable :: grada(:,:,:,:), gradmu(:,:,:)
       real*8 :: x, y, z, wgt, mu1, mu2, errtol_
       real*8 :: G(3,3,3)
!
       errtol_=ERRTOL()*50
!
...
! verify directly that matrix orthonormal:
        write(666,*) whoami, '> U x U^T (should be I):'
        write(666,'(3E22.15)') matmul(a,transpose(a)) ! aa

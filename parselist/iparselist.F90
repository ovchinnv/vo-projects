...
!
        do
         strl=min(max(0,strl),len(str)) ;str(strl+1:)='';str=adjustl(str(1:strl));strl=len_trim(str)
...
!! allow (almost) any value of STEP for flexibility
     ...
     & ' INVALID STEP VALUE SPECIFIED. USING STEP=1.')
              kstep=1
             endif
...
!
            if (inode.ne.missing.and.jnode.ne.missing) then
             ...
             enddo
              ...
              call wrndie(0,whoami, &
     & ' INVALID RANGE SPECIFIED. SKIPPING ENTRY.')
            endif
         endif ! i

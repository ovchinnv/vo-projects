...
float computeOnDevice(float* h_data, int num_elements)
{
  ...
  cudaMalloc((void**)&d_data, /* COMPLETE THIS FUNCTION CALL */);

  // * Copy input data from host memory to CUDA device memory
  cudaMemcpy(d_data, h_data, /* COMPLETE THIS FUNCTION CALL */);
  ...
  cudaMemcpy(&result, d_data, /* COMPLETE THIS FUNCTION CALL */ ); 
  cudaFree(d_data);
  return result;
}

...
%integ='lsq2c'; %the solution is computed at center (assuming f is at corners, i.e. offset) ; can reinterpolate onto the corner grid
%integ='lsq2'; %the solution is computed at same grid
%integ='lsq4c'; %4th order FD offset grid
integ='lsq4a'; %4th order FD same grid
...
% x-grad
   ...
   M(o,l)=-9/8 ;
   M(o,ll)=1/24 ;
   rhs(o) = ( 9*(fx(i,j) + fx(im,j)) - (fx(ip,j) + fx(imm,j)) ) / 16 * dx(1);
% y-grad
   M(2*o,t)=-1/24 ;
   M(2*o,o)=9/8 ;
   M(2*o,b)=-9/8 ;
   M(2*o,bb)=1/24 ;
   rhs(2*o)=( 9*(fy(i,j) + fy(i,jm)) - (fy(i,jp) + fy(i,jmm)) ) / 16 * dy(1);
%if (o==2) ;return;end
  end % nphi
 end % npsi
% M(end+1,:)=1 ; rhs(end+1)=0; % average solution
...
% y-grad
   ...
   M(o,b)=-9/8 ;
   M(o,bb)=1/24 ;
   rhs(o)+= ( 9*(fy(i,j) + fy(i,jm)) - (fy(i,jp) + fy(i,jmm)) ) / 16 * dy(1);
%if (o==2) ;return;end
  end % nphi
 end % npsi
 M(end+1,:)=1 ; rhs(end+1)=0; % average solution

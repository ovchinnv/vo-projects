...
def convertOpenMMPrefix(name):
    return (name.replace('DynamoPlugin::', 'OpenMM_')).replace('OpenMM::', 'OpenMM_')

#OPENMM_RE_PATTERN=re.compile("(.*)OpenMM:[a-zA-Z:]*:(.*)")
OPENMM_RE_PATTERN=re.compile("(.*)DynamoPlugin:[a-zA-Z:]*:(.*)")
...
class CHeaderGenerator(WrapperGenerator):
    ...
    def writeGlobalConstants(self):
        ...
        node = next((x for x in findNodes(self.doc.getroot(), "compounddef", kind="namespace") if x.findtext("compoundname") == "DynamoPlugin"))
    ...
    def writeMethods(self, classNode):
        ...
        if not ('abstract' in classNode.attrib and classNode.attrib['abstract'] == 'yes'):
            ...
            for methodNode in methodList:
                ...
                if methodName == shortClassName:
                    ...
                    self.out.write("extern OPENMM_EXPORT_DYNAMO %s* %s_create%s(" % (typeName, typeName, suffix))
                    self.writeArguments(methodNode, False)
                    self.out.write(");\n")
    
        # Write destructor
        self.out.write("extern OPENMM_EXPORT_DYNAMO void %s_destroy(%s* target);\n" % (typeName, typeName))
        ...
        for methodNode in methodList:
            ...
            if isConstMethod and any(methodNames[m] == methodName and m.attrib['const'] == 'no' for m in methodList):
                ...
                continue
            returnType = self.getType(getText("type", methodNode))
            self.out.write("extern OPENMM_EXPORT_DYNAMO %s %s_%s(" % (returnType, typeName, methodName))
    ...
    def writeOutput(self):
        print("""
#ifndef DYNAMO_OPENMM_CWRAPPER_H_
#define DYNAMO_OPENMM_CWRAPPER_H_

#ifndef OPENMM_EXPORT_DYNAMO
#define OPENMM_EXPORT_DYNAMO
#endif""", file=self.out)
...
OPENMM_EXPORT_DYNAMO OpenMM_3D_DoubleArray* OpenMM_3D_DoubleArray_create(int size1, int size2, int size3);
OPENMM_EXPORT_DYNAMO void OpenMM_3D_DoubleArray_set(OpenMM_3D_DoubleArray* array, int index1, int index2, OpenMM_DoubleArray* values);
OPENMM_EXPORT_DYNAMO void OpenMM_3D_DoubleArray_destroy(OpenMM_3D_DoubleArray* array);""", file=self.out)
...
#endif /*DYNAMO_OPENMM_CWRAPPER_H_*/""", file=self.out)
...
class CSourceGenerator(WrapperGenerator):
    ...
    def writeMethods(self, classNode):
            ...
            for methodNode in methodList:
                ...
                if methodName == shortClassName:
                    ...
                    if numConstructors == 1:
                        ...
                        suffix = "_%d" % numConstructors
                    self.out.write("OPENMM_EXPORT_DYNAMO %s* %s_create%s(" % (typeName, typeName, suffix))
                    self.writeArguments(methodNode, False)
                    self.out.write(") {\n")
        ...
        # Write destructor
        self.out.write("OPENMM_EXPORT_DYNAMO void %s_destroy(%s* target) {\n" % (typeName, typeName))
        ...
        for methodNode in methodList:
            ...
            self.out.write("OPENMM_EXPORT_DYNAMO %s %s_%s(" % (returnType, typeName, methodName))
            isInstanceMethod = (methodNode.attrib['static'] != 'yes')
            if isInstanceMethod:
                if isConstMethod:
                    self.out.write('const ')
...
#include "DynamoOpenMMCWrapper.h"
#include <cstring>
#include <vector>
...
/* OpenMM_2D_IntArray */
OPENMM_EXPORT_DYNAMO OpenMM_2D_IntArray* OpenMM_2D_IntArray_create(int size) {
    return reinterpret_cast<OpenMM_2D_IntArray*>(new vector<vector<int> >(size));
}
OPENMM_EXPORT_DYNAMO void OpenMM_2D_IntArray_destroy(OpenMM_2D_IntArray* array) {
    delete reinterpret_cast<vector<vector<int> >*>(array);
}
OPENMM_EXPORT_DYNAMO int OpenMM_2D_IntArray_getSize(const OpenMM_2D_IntArray* array) {
    return reinterpret_cast<const vector<vector<int> >*>(array)->size();
}
OPENMM_EXPORT_DYNAMO void OpenMM_2D_IntArray_resize(OpenMM_2D_IntArray* array, int size) {
    reinterpret_cast<vector<vector<int> >*>(array)->resize(size);
}
OPENMM_EXPORT_DYNAMO void OpenMM_2D_IntArray_append(OpenMM_2D_IntArray* array, int index1, int value) {
...
OPENMM_EXPORT_DYNAMO void OpenMM_2D_IntArray_set(OpenMM_2D_IntArray* array, int index1, int index2, int value) {
    vector<vector<int> >* array2DInt = reinterpret_cast<vector<vector<int> >*>(array);
    if (array2DInt->size() <= index1) {
        array2DInt->resize(index1+1);
    }
...
OPENMM_EXPORT_DYNAMO void OpenMM_2D_IntArray_get(const OpenMM_2D_IntArray* array, int index1, int index2, int* value) {
...
OPENMM_EXPORT_DYNAMO void OpenMM_3D_DoubleArray_destroy(OpenMM_3D_DoubleArray* array) {
...
class FortranHeaderGenerator(WrapperGenerator):
    ...
    def __init__(self, inputDirname, output):
        ...
        self.typeTranslations = {'int': 'integer*4',
                                 ...
                                 'OpenMM::System': 'type (OpenMM_System)',
                                 'OpenMM::ContextImpl': 'type (OpenMM_Context)'}
        ...
        if not ('abstract' in classNode.attrib and classNode.attrib['abstract'] == 'yes'):
            ...
            for methodNode in methodList:
                ...
                if methodName == shortClassName:
                    ...
                    self.out.write(")\n")
                    self.out.write("            use OpenMM_Types; use OpenMMDynamo_Types; implicit none\n")
...
MODULE OpenMMDynamo
    use OpenMM
    use OpenMM_Types
    use OpenMMDynamo_Types; implicit none
    interface
""", file=self.out)
...
END MODULE OpenMMDynamo""", file=self.out)
...
class FortranSourceGenerator(WrapperGenerator):
    ...
    def writeOneConstructor(self, classNode, methodNode, functionName, wrapperFunctionName):
        ...
        shortClassName = stripOpenMMPrefix(className)
        typeName = convertOpenMMPrefix(className)
        self.out.write("OPENMM_EXPORT_DYNAMO void %s(%s*& result" % (wrapperFunctionName, typeName))
        self.writeArguments(methodNode, True)
        self.out.write(") {\n")
    ...
    def writeOneDestructor(self, typeName, wrapperFunctionName):
        self.out.write("OPENMM_EXPORT_DYNAMO void %s(%s*& destroy) {\n" % (wrapperFunctionName, typeName))
    ...
    def writeOneMethod(self, classNode, methodNode, methodName, wrapperFunctionName):
        ...
        if methodType in self.classesByShortName:
            methodType = self.classesByShortName[methodType]
        self.out.write("OPENMM_EXPORT_DYNAMO ")
        if hasReturnValue:
...
builder = CHeaderGenerator(inputDirname, open(os.path.join(sys.argv[2], 'DynamoOpenMMCWrapper.h'), 'w'))
builder.writeOutput()
builder = CSourceGenerator(inputDirname, open(os.path.join(sys.argv[2], 'DynamoOpenMMCWrapper.cpp'), 'w'))
builder.writeOutput()
builder = FortranHeaderGenerator(inputDirname, open(os.path.join(sys.argv[2], 'DynamoOpenMMFortranModule.f90'), 'w'))
builder.writeOutput()
builder = FortranSourceGenerator(inputDirname, open(os.path.join(sys.argv[2], 'DynamoOpenMMFortranWrapper.cpp'), 'w'))
builder.writeOutput()
